...
- [ ] **Fix planner output for recurring script schedules** — Live `npx geeksy` browser test for “tell me a joke each minute” still sometimes writes invalid recurring scripts/imports or duplicates schedules instead of producing one clean self-contained interval script.
...
- [ ] **Bind scheduled run output to the owning conversation** — Successful/failing script outputs still need to land in the correct active session reliably, not agent-global/null scope.
