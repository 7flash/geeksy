...
- [ ] **Improve heartbeat pruning quality beyond blunt message trimming** — The deterministic trim unblocks runtime, but it does not yet write a durable summary/core-memory artifact before dropping old messages.
...
- [ ] **Live browser-proof the masked secret request flow** — Backend coverage now verifies `/api/secrets` metadata-only reads and `/api/secrets/submit` sanitized chat persistence. Remaining work is end-to-end web UI proof that a `request_secret` tool call renders the masked chat card, `Save & continue` resumes the bound conversation, and no raw secret value leaks into visible chat/tool output.
