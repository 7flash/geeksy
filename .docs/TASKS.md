...
- [ ] **Live browser-proof the masked secret request flow** — Backend coverage now verifies `/api/secrets` metadata-only reads, `/api/secrets/submit` sanitized chat persistence, and `/api/secrets/request` marker injection. Settings now has an `Add masked prompt to chat` trigger so the end-to-end web UI proof can be exercised deterministically without waiting for LLM/tool luck.
