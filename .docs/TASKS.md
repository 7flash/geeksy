...
- [ ] **Reduce heartbeat schedule-tool chatter** — Heartbeat now binds recurring schedules to the correct conversation, and the noise filter now suppresses raw JSON plus low-signal `schedule list` / `nothing needs attention` boilerplate. Remaining work is live verification that new heartbeat ticks no longer append this chatter to conversations.
