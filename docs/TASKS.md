...
- [ ] **Make heartbeat core memory session-aware** — `core_memory` is currently agent-level state; if multiple conversations matter, the retained summary may need per-conversation scoping or provenance.
