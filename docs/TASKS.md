...
- [ ] **Live-check session-scoped core memory rendering after a real prune** — Confirm in the browser that switching conversations changes the pinned `Heartbeat Core Memory` card when multiple session-scoped summaries exist.
